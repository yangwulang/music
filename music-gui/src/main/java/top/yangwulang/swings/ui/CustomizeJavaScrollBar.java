 ...
 * @author yangwulang
 */
public class CustomizeJavaScrollBar extends BasicScrollBarUI {
    private final int width;
    private int radius;

    public CustomizeJavaScrollBar() {
        this.width = 12;
    }

    public CustomizeJavaScrollBar(int width) {
        this.width = width;
    }

    public CustomizeJavaScrollBar(int width, int radius) {
        this.width = width;
        this.radius = radius;
    }
...
//         thumbColor = Color.GRAY;
    ...
    public void paintTrack(Graphics g, JComponent c, Rectangle trackBounds) {
        ...
        if (this.scrollbar.getOrientation() == JScrollBar.VERTICAL) {
            //设置画笔
            gp = new GradientPaint(0, 0, new Color(130, 130, 130),
                    trackBounds.width, 0, new Color(207, 207, 207));
        }
        if (this.scrollbar.getOrientation() == JScrollBar.HORIZONTAL) {
            gp = new GradientPaint(0, 0, new Color(130, 130, 130),
                    trackBounds.height, 0, new Color(207, 207, 207));
        }
    ...
    protected void paintThumb(Graphics g, JComponent c, Rectangle thumbBounds) {
        ...
        g.drawRoundRect(0, 0, width, thumbBounds.height - 1, radius, radius);
        // 消除锯齿
        Graphics2D g2 = (Graphics2D) g;
        ...
        // 填充圆角矩形
        g2.fillRoundRect(0, 0, width, thumbBounds.height - 1, radius, radius);
    }
